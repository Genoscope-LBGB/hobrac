...
def _build(tmp_path):
    ...
    # g2 is ABSENT in Bf; g4 is ABSENT in Hl.
    species_busco = [
        ("Pm", {"g1": _gene("chr1"), "g2": _gene("chr1"), "g3": _gene("chr9"),
                "g4": _gene("chr2")}),
        ("Bf", {"g1": _gene("chr3"), "g3": _gene("chr8"), "g4": _gene("chr5")}),
        ("Hl", {"g1": _gene("chr20"), "g2": _gene("chr20"), "g3": _gene("chr7")}),
    ]
...
class TestSaveGeneChains:
    def test_header(self, tmp_path):
        ...
        assert rows[0] == ["chain_id", "gene", "Pm", "Bf", "Hl", "color"]
    ...
    def test_chain_id_and_color(self, tmp_path):
        rows = {r[1]: r for r in _build(tmp_path)[1:]}
        assert rows["g1"][0] == "0"
        assert rows["g1"][-1] == "#1f77b4"
        assert rows["g3"][0] == "1"
        assert rows["g3"][-1] == "#ff7f0e"
        # No-chain gene: chain_id -1, lightgrey.
        assert rows["g4"][0] == "-1"
        assert rows["g4"][-1] == "lightgrey"
